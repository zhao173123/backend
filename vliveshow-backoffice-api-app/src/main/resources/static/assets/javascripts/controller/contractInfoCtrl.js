...
backStart.controller("contractInfoCtrl",["$scope","$rootScope","DateInit", "$stateParams", "$http", "$window", "$sce", "fileReader", "Upload", "PubFunction", "SearchService",
    function($scope, $rootScope,DateInit,$stateParams, $http, $window, $sce, fileReader, Upload, PubFunction, SearchService){ // 合约资料
        ...
        $scope.manageTypeList = [{id:1, value:'自制'}, {id:2, value:'运营'}];
        $scope.applyTypeList = [{id:1, value:'邀约'}, {id:2, value:'自主申请'}];
        ...
        var dataWareUp = function (contractInfo) {
            ...
            $scope.baseSalary.currency = angular.isDefined(contractInfo.baseSalary) && contractInfo.baseSalary.hasOwnProperty('currency') ?  contractInfo.baseSalary.currency : $scope.manageTypeList[0].id;
            $scope.managementTeamType = contractInfo.hasOwnProperty('managementTeamType') ?  contractInfo.managementTeamType : '';
            $scope.applyType = contractInfo.hasOwnProperty('applyType') ?  contractInfo.applyType : '';
        ...
        **/
        ...
        $scope.inputValidate = function () {
          if($scope.selectedTalent.length === 0 || $scope.requiredHours === '' || $scope.baseSalary.amount === ''|| $scope.platformShareRate === '' || $scope.settlementBase === '' ||
               $scope.settlementDueDay === '' ||  $scope.directorId === 0 || $scope.supervisorId === 0 || $scope.agentId === 0 || $scope.bankName === '' ||
                $scope.bankAccount === '' || $scope.startDate === '' || $scope.endDate === '' || $scope.managementTeamType === '' || $scope.applyType === ''){
              return true;
          }else {
              return false;
          }
        ...
        $scope.save = function () {
            ...
            reqPayload.managementTeamType = $scope.managementTeamType;
            reqPayload.applyType = $scope.applyType;
