backStart
    .controller("powerInfoCtrl", ['$scope','$rootScope', '$http','$window', 'SearchService', 'CurrentUser', '$stateParams', function($scope, $rootScope, $http, $window, SearchService, CurrentUser, $stateParams){ // 关注用户列表
        //获得当前查看的用户的信息
        ...
        $scope.submitRole = function(){
            ...
            if(submitArr.length == 0){ //没有改变则不提交
               // console.log("没有修改角色 ----- 保存成功~   ----->" + submitArr);
            ...
            $scope.getRoleList({  //提交
                ...
                //console.log("角色做了修改 ----- 保存成功~  ----->" + submitArr);
                SearchService.getTips("保存成功");
                // alert("保存成功！~");
        ...
        //初始化
        $scope.init($rootScope.currentUserInfo.role);
