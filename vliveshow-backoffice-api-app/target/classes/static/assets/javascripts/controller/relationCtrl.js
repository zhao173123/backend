backStart
    .controller("relationCtrl", ['$scope','$rootScope', '$http', '$window', '$state', '$stateParams','PubFunction', 'SetNavRight','SearchService','GetList','DataCtrl', 'DataSort', 'DateInit', 'ExportToCsvService',
        function($scope, $rootScope, $http, $window, $state, $stateParams, PubFunction, SetNavRight, SearchService, GetList, DataCtrl, DataSort, DateInit, ExportToCsvService){ // 用户关系管理
            ...
            // var loadTest = function (arr, callback) {
            //     var resArr = [];
            //     for(var i=0; i<arr.length; i++){
            //         $http({
            //             // url : $scope.hostName + "v1/backoffice/users/list?token=" + $scope.tokenCode + "&pageSize=10&pageNum=1",
            //             url : $scope.hostName + "v1/backoffice/users/detail?token=" + $scope.tokenCode + "&userId=" + arr[i],
            //             // url : "http://124.172.174.187:8888/vliveshow-backoffice-app/v1/backoffice/users/list" + "?token=6a8860892b7710dbba7bba85aa4a90ce" + "&pageSize=10" + "&pageNum=1" + (params.urlParams?params.urlParams:""),  //params.url
            //             method : "GET",
            //             // data : {userId: arr[i]}, //请求提交数据
            //             headers:{
            //                 'Content-Type': 'application/json'
            //             }
            //         }).success(function (data) {
            //             // var result = data.results.list[0];
            //             var result = data.results.user_detail;
            //             if(angular.isDefined(result) && result.hasOwnProperty('userId')){
            //                 // var redata = {userId: data.results.list[0].userId, nickName: data.results.list[0].nickName, mobilePhone: data.results.list[0].mobilePhone,create_time: data.results.list[0].create_time };
            //                 var redata = {userId: result.userId, nickName: result.nickName, mobilePhone: result.mobilePhone,create_time: result.create_time, IDFA: result.IDFA };
            //                 resArr.push(redata);
            //             }else{
            //                 resArr.push(result);
            //             }
